    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(dianyingOrder !=null){
            ...
            BeanUtils.copyProperties( dianyingOrder , view );//把实体数据重构到view中
                ...
                //级联表
                DianyingChangciEntity dianyingChangci = dianyingChangciService.selectById(dianyingOrder.getDianyingChangciId());
                if(dianyingChangci != null){
                    BeanUtils.copyProperties( dianyingChangci , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setDianyingChangciId(dianyingChangci.getId());
                }
                //级联表
...
//                            dianyingOrderEntity.setDianyingChangciId(Integer.valueOf(data.get(0)));   //电影场次 要改的
...
//                            dianyingOrderEntity.setCreateTime(date);//时间
        ...
        DianyingOrderEntity dianyingOrder = dianyingOrderService.selectById(id);
            if(dianyingOrder !=null){
                ...
                //级联表
                    DianyingChangciEntity dianyingChangci = dianyingChangciService.selectById(dianyingOrder.getDianyingChangciId());
                if(dianyingChangci != null){
                    BeanUtils.copyProperties( dianyingChangci , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setDianyingChangciId(dianyingChangci.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(dianyingOrder.getYonghuId());
    ...
    */
    ...
    public R commentback(Integer id, String commentbackText, Integer dianyingCommentbackPingfenNumber, HttpServletRequest request){
        ...
        if(dianyingOrder.getDianyingOrderTypes() != 4)
